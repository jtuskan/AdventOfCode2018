...
#..G#E#
#.....#
#######`.split("\n").filter(i => i).reduce((matrix, row) => {
	matrix.push(row.trim().split(''));
	return matrix;
}, []);
var graph = new Graph();
var units = [];
var nodeIndex = (x, y) => `${x}|${y}`;
map.forEach((row, y) => {
	row.forEach((node, x) => {
		if (node !== "#") {
			var current = nodeIndex(x, y);
			graph.addNode(current);
			if (map[y-1] && map[y-1][x] === ".") { 
				graph.addEdge(current, nodeIndex(x, y-1))
            }

			if (map[y+1] && map[y+1][x] === ".") graph.addEdge(current, nodeIndex(x, y+1))
			if (map[y][x-1] && map[y][x-1] === ".") graph.addEdge(current, nodeIndex(x-1, y))
			if (map[y][x+1] && map[y][x+1] === ".") graph.addEdge(current, nodeIndex(x+1, y))
			if (node === "G") addUnit(x, y, node);
			if (node === "E") addUnit(x, y, node); 
        }
    })
})
var addUnit = (x, y, type) => units.push({ type, x, y, health: 200, damage: 3 })


var elves = units.filter(x => x.type === "E");
units.filter(i => i.type === "G").map(i => {
	var distances = elves.map(e => ({
		weight: graph.shortestPath(nodeIndex(i.x, i.y), nodeIndex(e.x, e.y)).weight,
		...e
    }));
	var minimum = Math.min.apply([], distances.map(d => d.weight));
	var closest = distances.filter(d => d.weight === minimum).sort((a, b) => a.y - b.y)[0];
...
function Graph(serialized){
    ...
    function shortestPath(source, destination){
      ...
      function path(){
        ...
        if (node !== source) {
          return { weight: Number.MAX_SAFE_INTEGER };
        }
        nodeList.push(node);
        nodeList.reverse();
